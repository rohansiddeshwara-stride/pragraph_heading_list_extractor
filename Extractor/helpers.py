...
def get_font_info(lines):
    ...
    for block in lines:
      ...
      if "lines" in block:
        ...
        font_size = statistics.mode(font_size)
        # print(font_type, spans)
        overall_font_size.append(font_size)
        block_info[block["number"]] = [font_type, font_size, lines_]
    avg_font_size = statistics.mode(overall_font_size)
    return block_info, avg_font_size
...
def get_paras_headings(blocks, avg_font_size):
    ...
    heading = []
    for block in blocks: #for each block
        if  not block[2].isupper() and len(block[4]) > 5 and not re.search("Bold", block[5]):
          # check if the block doesnot contains text which is all upper and no of words in the block > 10 and if the text is not bold
          para.append(block)
        # else:
        #   print(block[2].isupper(),len(block[4]) > 5,block[5], block[2])
        elif  block[2].isupper() or re.search("Bold", block[5]) or block[6] > avg_font_size :
          # check if the block doesnot contains text which is all upper and no of words in the block > 10 and if the text is not bold
    ...
    for line in lines:
      ...
      if not line["text"].isspace():
        if re.search("^[•●○*◦▪▫■◆◘‣⁌❖⮞⬑➔⇒➢✓]", line["text"]):
          ...
          if (math.floor((origin[-1][1])) == math.floor(line["origin"][1])) or (math.floor(o[0]) == math.floor(line["origin"][0])) or (math.floor(o[1]) == math.floor(line["origin"][1])):
...
def get_numbered_list(blocks):
    ...
    for line in lines:
      ...
      line_no+=1
      if not line["text"].isspace():
        if re.search("^(((M{0,4}(CM|CD|D?C{0,3})(XC|XL|L?X{0,3})(IX|IV|V?I{0,3}))&(\.|\ ))|([0-9]{1,2}\.|[a-zA-Z]\.\ |\(\ ?[a-zA-Z]\ ?\))|((m{0,4}(cm|cd|d?c{0,3})(xc|xl|l?X{0,3})(ix|iv|v?i{0,3}))&(\.|\ )))", line["text"]):
          if text!='':
            new_blocks.append([get_block_bbox(bbox), page_no,text, line["page_size"]])
...
# def get_headings(blocks, avg_font_size):

#     new_blocks = []
#     # print("here", avg_font_size)
#     for block in blocks: #for each block
#         # print(block)
#         # print(block[2], block[6])
#         if  block[2].isupper() or re.search("Bold", block[5]) or block[6] > avg_font_size :
#           # check if the block doesnot contains text which is all upper and no of words in the block > 10 and if the text is not bold
#           new_blocks.append(block)
