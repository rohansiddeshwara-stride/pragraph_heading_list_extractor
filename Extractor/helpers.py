...
def drop_tables(blocks):
  ...
  '''
  this function takes a block of text and based on dots i.e . and spacing between each words of the same line
  checks if its a table or TOC and drops those blocks
  Input : blocks - text block dataset created earlier
  Output : new_blocks - text blocks without tables and TOC
  '''

  new_blocks = []
  for block in blocks: #for each block

    if not re.search("[.]{3,}", block[2]): #check if it doesnt have more than 3 dots cuz its mostly TOC
      avg_space = []
      ideal_space = 25
      if len(block[4]) == 1:
      ...
      for i in range(len(block[4]) - 1): # for each word in each block
        if math.floor(block[4][i][3]) == math.floor(block[4][i + 1][3]): #check if the consecutive words are in the same line.
          avg_space.append(abs(block[4][i][2] - block[4][i + 1][0])) # if yes calclate the sapce betwween the words

      if len(avg_space) != 0:
        block_avg_space = sum(avg_space)/ len(avg_space)

        if block_avg_space <= ideal_space or (len(block[4]) > 1 and len(avg_space) == 0):
          #if the avg space between words is less than ideal space then proceed
          new_blocks.append(block)
...
def group_text_to_blocks(words):
  ...
  '''
  link : https://pymupdf.readthedocs.io/en/latest/app1.html
  this function takes individual words present in a page
  and groups them based on the block number hence getting individual words present in a block
  Input : words - tuple of individual words present in a page format (x0, y0, x1, y1, "word", block_no, line_no, word_no)
  Output : list_of_lists - list of lists where each list is list of tuples of each word present within a block which is the index no 
  where each tuple follows (x0, y0, x1, y1, "word", block_no, line_no, word_no) format
  ...
  for tup in words:
      key = tup[5]  # Use the block number to 
...
def get_paras_headings(blocks, avg_font_size):
  ...
  for block in blocks: #for each block
      ...
      if  not block[2].isupper() and len(block[4]) > 7 and not re.search("Bold", block[5]) and not re.search("^[•●○*◦▪▫■◆◘‣⁌❖⮞⬑➔⇒➢✓-]", block[2]) and block[6]<=avg_font_size+1 and re.search("([a-zA-Z]+)", block[2]):
        # check if the block doesnot contains text which is all upper and no of words in the block > 10 and if the text is not bold
        para.append(block)

      elif  ((block[2].isupper() and block[6] > avg_font_size) or block[6] >= avg_font_size + 2  or re.search("Bold", block[5]))  and not re.search("^[•●○*◦▪▫■◆◘‣⁌❖⮞⬑➔⇒➢✓-]", block[2]) and re.search("([a-zA-Z]+)", block[2]):
        # check if the block doesnot contains text which is all upper and no of words in the block > 10 and if the text is not bold
  ...
  for line in lines:
    ...
    if not line["text"].isspace():
      if re.search("^[•●○*◦▪▫■◆◘‣⁌❖⮞⬑➔⇒➢✓-]", line["text"]):
        # print("1", line["text"])
        if len(text) >=3 and re.search(text_patt, text):
          # print("2", text)
        ...
        # print(line["origin"], o, (abs((o[1]) - line["origin"][1]) <=3),(math.floor(o[0]) + 10<= math.floor(line["origin"][0])),(math.floor(o[0]) + 20>= math.floor(line["origin"][0])),( math.floor(origin[-1][1]) + 20>= math.floor(line["origin"][1])),line["page_no"] == page_no and ((abs((o[1]) - line["origin"][1]) <=3) or ((math.floor(o[0]) + 10<= math.floor(line["origin"][0])) and (math.floor(o[0]) + 20>= math.floor(line["origin"][0])) and ( math.floor(origin[-1][1]) + 20>= math.floor(line["origin"][1])))), line["text"])
        if line["page_no"] == page_no and ((abs((o[1]) - line["origin"][1]) <=3) or ((math.floor(o[0]) + 10<= math.floor(line["origin"][0])) and (math.floor(o[0]) + 20>= math.floor(line["origin"][0])) and ( math.floor(origin[-1][1]) + 20>= math.floor(line["origin"][1]))) or (abs((origin[-1][1]) - line["origin"][1]) <=3)):
# check if the two lines are in same page and the new lines y origin matches with the previos lines y origin
# or if the new line is indented within a range of the previous line both in x and y direction
#or if it aligns in y direction with the last appended word
            origin.append(line["origin"])
            # o = origin[1]
            text+= ' ' + line["text"]
            bbox.append(line["bbox"])

        elif len(text) >=3 and re.search(text_patt, text):
          # print("3", text)
          new_blocks.append([get_block_bbox(bbox), page_no,text, line["page_size"]])
          text = ''
          bbox = []
          origin = []
      if line_no == len(lines) and len(text) >=3 and re.search(text_patt, text):
        # print("4", text)
        new_blocks.append([get_block_bbox(bbox), page_no,text, line["page_size"]])
...
def get_numbered_list(blocks):
  ...
  for line in lines:
    ...
    line_no+=1
    if not line["text"].isspace():
      if re.search("^(((M{0,4}(CM|CD|D?C{0,3})(XC|XL|L?X{0,3})(IX|IV|V?I{0,3}))&(\.|\ ))|([0-9]{1,2}\.|[a-zA-Z]\.\ |\(\ ?[a-zA-Z]\ ?\))|((m{0,4}(cm|cd|d?c{0,3})(xc|xl|l?X{0,3})(ix|iv|v?i{0,3}))&(\.|\ ))|[0-9]{1,2})", line["text"]) :
        if len(text) >=3 and re.search(text_patt, text):
          new_blocks.append([get_block_bbox(bbox), page_no,text, line["page_size"]])
